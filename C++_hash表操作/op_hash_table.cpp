...
static void _delete(std::unordered_map<std::string, int>& table, std::string key, int result) {table.erase(key);}
static void _print(std::unordered_map<std::string, int>& table, std::string key, int result) {printf("print:\n    {%s: %d}\n", key.c_str(), result);}

template <typename F, typename T>
static void process_entry_in_hash_table(F cb, T& table,  std::string key) {
...
static void process_entry_in_hash_table(F cb, T& table, std::string key, Args... args) {
	_HASH_SEARCH_START(table, field, key, result){
