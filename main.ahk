...
找到图片位置后点击(图片, 操作参数)
{
    偏移量x := IsNumber(操作参数[1]) ? 操作参数[1] : 找到图片位置后偏移量x
    偏移量y := IsNumber(操作参数[2]) ? 操作参数[2] : 找到图片位置后偏移量y
    宽度 := 高度 := 目标坐标x := 目标坐标y := 0
    左上角x := 左上角y := -200
...
依次查找图片并点击(操作参数*)
{
    ...
    if 校验类型(图库, 字符串类型)
    {
        ...
        loop parse, 图库, "`n", "`r"
        {
            图片 := A_LoopField
            断点 "对文件进行操作: " 图片
            对文件进行操作 图片, "找到图片位置后点击", 相对偏移量, 是否忽略找不到图片的错误
            休息 一小小会儿
        }
    }
    if 校验类型(图库, 数组类型)
...
依次查找图片并输入文本(操作参数*)
{
    ...
    if 校验类型(图文库, 字符串类型)
    {
        ...
        loop parse, 图文库, "`n", "`r"
        {
            ...
            对文件进行操作 图片, "找到图片位置后输入文本", [相对偏移量, 文字], 是否忽略找不到图片的错误
            休息 一小小会儿
        }
    }
    if 校验类型(图文库, 数组类型)
